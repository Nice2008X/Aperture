import type { ModelEdge } from "@aperture/model-ir";
 ...
 * llm-len/LLM-Explorer's own elkLayout.ts, which this was ported from, and
 * the .scratch smoke tests it cites): on SOUTH (a node's outgoing ports),
 * ascending index runs RIGHT to LEFT, so matching creation order (first
 * edge reads leftmost, as every adapter assumes) requires reversing the
 * index; on NORTH (incoming ports), ascending index already runs LEFT to
 * RIGHT, so creation order maps directly with no reversal. `flip` inverts
 * this once more — used by the crossing-retry loop below.
 */
...
async function runElk(
  ...
  // own chunk instead of inflating the app's existing main bundle — it
  // only loads once a graph actually needs laying out.
 ...
 * (`label: "skip"`) edges are excluded from ELK's own graph entirely — they
 ...
 * so a general-purpose routing engine has nothing to add there. Every other
 * edge is routed by ELK, including what used to need this app's own
 * hand-rolled "detour"/"hub lane" special cases in ArchitectureGraph.tsx
 * (an edge skipping over an intervening rank) — ELK's layered algorithm
 * already inserts the dummy nodes a long edge needs to route around
 * intervening ranks, which is exactly what those hand-rolled cases were
 * working around one topology at a time.
 *
 ...
 * started. One `ELK.layout()` call is cheap on graphs this size, so rather
 * than build a bespoke local-search optimizer, this retries with each
 * individual multi-port node's fixed order flipped, one at a time, and
 * keeps whichever attempt has the fewest real (geometric, not topological)
 * crossings — bounded to one extra ELK call per multi-port node, which is
 * always a small count for a single transformer block.
 */
export async function computeElkLayout(nodeIds: string[], edges: ModelEdge[], sizes: Map<string, NodeSize>, fallbackSize: NodeSize): Promise<ElkLayoutResult> {
  ...
  const routed = relevant.filter((e) => e.label !== "skip");
  ...
  // the DFS-reversal bug this depends on).
