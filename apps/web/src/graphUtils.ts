 ...
 *
 * A second, independent pass handles the one repeated-node shape that
 * *isn't* a chain: an MoE layer's `expert` children, which all fan out in
 * parallel from the same router (and back into the same combine step)
 * rather than feeding one into the next — a branch/merge that the chain
 * pass above deliberately treats as a boundary, not a run. Scoped tightly
 * to `NodeType === "expert"` rather than "any same-type siblings sharing a
 * parent": a block's own res1/res2 (both type `residual`, both children of
 * the same block) are two genuinely different steps that would otherwise
 * wrongly collapse into one, so this only ever groups the one node type
 * that's actually a repeated-by-construction sibling set.
 */
export function collapseRepeatedChains(
  ...
  for (const id of nodeIds) {
    if (!isRealNode(id) || replaced.has(id) || model.nodes[id].type !== "expert") continue;
    const parentId = model.nodes[id].parentId ?? "";
    if (!expertGroups.has(parentId)) expertGroups.set(parentId, []);
    expertGroups.get(parentId)!.push(id);
  }
  for (const [parentId, members] of expertGroups) {
    if (members.length < 2) continue;
    const stackId = `${STACK_PREFIX}${parentId}.experts`;
    stacks.set(stackId, { memberIds: members, type: "expert" });
    for (const m of members) replaced.set(m, stackId);
  }
