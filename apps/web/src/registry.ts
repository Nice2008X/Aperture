import type { NodeType } from "@aperture/model-ir";
...
export const componentRegistry: Record<NodeType, NodeTypeInfo> = {
  ...
  moe_layer: {
    ...
    color: "#7c3aed",
    description: "Replaces a dense feed-forward network with many smaller expert networks. A router picks a handful of experts per token, so each token is processed by only a fraction of the layer's total parameters.",
    category: "structural",
  },
  router: {
    label: "Router",
    color: "#f59e0b",
    description: "Scores every expert for each token and picks the top few to activate, weighting their outputs by how confident the router was.",
    formula: "weights = softmax(x W_router), top_k selected",
    category: "linear",
  },
  expert: {
    label: "Expert",
    color: "#7c3aed",
    description: "One of an MoE layer's parallel feed-forward networks — structurally the same as a dense FFN, but only run for the tokens routed to it.",
    category: "other",
  },
  custom: {
    label: "Custom",
    color: "#94a3b8",
    description: "A module the graph builder didn't recognize as a named component. Shown as-is (with its real shape and parameters) rather than dropped.",
    category: "other",
  },
};
