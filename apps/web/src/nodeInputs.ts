 ...
 * activation is its own *output* (see every hook registration in
 * `inference.py`), containers included. So `activations[container]` is that
 * block's result, not what fed into it — using it directly as "this
...
function resolveBoundarySources(model: Model, containerId: string): InputSource[] {
  ...
  return upstream.map((e) => {
    ...
    return { label: `Block input (from ${name})`, isBlockBoundary: true, sourceId: e.source };
  });
}
