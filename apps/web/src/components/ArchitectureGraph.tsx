...
import type { Model, ModelNode } from "@aperture/model-ir";
 ...
 * side and down a vertical lane never has anything to cross — this is the
 * one routing case that never needed a general-purpose layout engine, so
 * it's kept exactly as before rather than routed through ELK too.
 */
...
/**
 * Every other edge: drawn through ELK's own computed route (a straight line
 * for an adjacent-rank edge, orthogonal bends for anything ELK had to route
 * around another node) instead of relying on React Flow's handle positions
 * at all — `data.points` is already in the same absolute flow-space
 * coordinates as node positions, straight from `computeElkLayout`. This is
 * what replaced this component's own hand-rolled obstacle-avoidance
 * detour/hub-lane system (see PLAN.md phase 15): ELK's layered algorithm
...
export function ArchitectureGraph({ model, view, selectedId, onSelect, onEnterBlock, onExitBlock, isMaxFrame, onToggleMaxFrame, onZoomChange }: Props) {
  ...
  // The one routing decision left outside ELK: every residual/skip edge in
  // a view shares a single lane, cleared against every node (and every
  ...
  const skipLaneX = useMemo(() => {
    const hasSkip = edgeList.some((e) => e.label === "skip");
    if (!hasSkip || !elkResult) return null;
    let maxRight = 0;
    for (const id of nodeIds) {
    ...
    () =>
      edgeList.map((e) => {
        ...
        return {
          ...
          source: e.source,
          target: e.target,
          sourceHandle: isSkip ? "lane-out" : undefined,
          targetHandle: isSkip ? "lane-in" : undefined,
          type: isSkip ? "lane" : "routed",
          data: isSkip ? { laneX: skipLaneX ?? undefined } : { points: elkResult?.routes.get(e.id)?.points ?? [] },
